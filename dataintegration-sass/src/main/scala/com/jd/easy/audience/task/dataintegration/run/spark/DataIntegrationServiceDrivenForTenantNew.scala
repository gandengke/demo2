...
import com.jd.easy.audience.common.constant.{NumberConstant, SplitDataSourceEnum, SplitDataTypeEnum, StringConstant}
...
import com.jd.easy.audience.common.util.{JdCloudOssUtil, JsonUtil}
import com.jd.easy.audience.task.commonbean.bean.{LoadData4TenantBean, SynData2TenantBean}
import com.jd.easy.audience.task.commonbean.contant.{BuffaloCircleTypeEnum, CATemplateEnum, SplitSourceTypeEnum}
...
import com.jd.easy.audience.task.dataintegration.util.{CustomSerializeUtils, SparkUtil, TableMetaUtil}
import com.jd.easy.audience.task.driven.run.{AbstractStepExecutor, StepExecutor}
 ...
 * 同步库的数据到租户库中-ca模版数据
 */
object DataIntegrationServiceDrivenForTenantNew {
  private val LOGGER = LoggerFactory.getLogger(DataIntegrationServiceDrivenForTenantNew.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    LOGGER.info("DataIntegrationServiceDrivenForSynDataTenantNew arg: {}", decodeJsonContext)
    ...
    try {
      ...
      if (StringUtils.isAnyBlank(ossUrl, dbName, accountName, accountId)) {
        LOGGER.error("DataIntegrationServiceDrivenForSynDataTenantNew argfile missing pipeline argument.: ")
        throw new JobInterruptedException("pipeline 的segment定义错误", "Wrong pipeline segment define")
      }
      val beanSegment: String = JdCloudOssUtil.getObject(jfsClient, jfsConf.getAppContextBucket, ossUrl)
      ...
      case e: Exception =>
        LOGGER.error("DataIntegrationServiceDrivenForSynDataTenantNew deserialize error: ", e)
    ...
    try {
      ...
      var outputMap: util.Map[String, Object] = null
      if (MapUtils.isNotEmpty(sparkSegment.getExtraDeal) && sparkSegment.getExtraDeal.containsKey(ConfigProperties.BUILD_SPLIT_BEANS_CLASS)) {
        //需要重新构造beans
        val buildClassName: String = sparkSegment.getExtraDeal.get(ConfigProperties.BUILD_SPLIT_BEANS_CLASS)
        val c = Class.forName(buildClassName)
        val stepObj = c.newInstance.asInstanceOf[IBeansMapBuilding]
        val beans: util.Map[String, StepCommonBean[_]] = stepObj.builtBeans(sparkSegment, accountId, accountName, dbName)
        LOGGER.info("beansMapNew=" + CustomSerializeUtils.serialize(beans))
        outputMap = StepExecutor.run(sparkSegment.getName, beans)
      } else {
        ...
        outputMap = StepExecutor.run(sparkSegment.getName, beansMap)
      }
