 ...
 */
public class SynPublicDataTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(SynPublicDataTest.class);
    ...
    public static void main(String[] args) {
        ...
        /**
         * wechat_onlien.json 企业微信数据同步
         *
         */
        SparkStepSegment sparkSegment = getsparkstepInfo("../syndata/wechat_online.json");
        try {
            //4.任务执行
            Object[] steps = sparkSegment.getBeans().keySet().toArray();
            for (int i = 0; i < steps.length; i++) {
                String stepName = steps[i].toString();
                SynData2TenantBean stepBean = (SynData2TenantBean) sparkSegment.getBeans().get(stepName);
...
//                stepObj.dealDataFilter(sparkSession, (SynData2TenantBean) stepBean, true);
    ...
    private static SparkStepSegment getsparkstepInfo(String argPath) {
        ...
        try {
            //2.1
            System.out.println(SynPublicDataTest.class.getResource("").getPath());
            File file = new File(SynPublicDataTest.class.getResource("").getPath() + argPath);
            if (file.exists()) {
                try {
                    // 将文件转换成String
