...
import com.jd.easy.audience.task.plugin.step.dataset.rfm.algorithms.result.LevelCrowdResult
...
import com.jd.easy.audience.task.plugin.step.dataset.rfm.algorithms.taskInfo.{RfmAnalysisConfigInfo, RfmAnalysisTask}
...
object RfmAnalysisOld {
  ...
  def computeImpl(spark: SparkSession,
                  jfsBucket: String,
                  taskGroup: Array[RfmAnalysisTask]
    ...
    var orderData = spark.emptyDataset[(String, Long, Double, Int, String, String, Long)]
    if (orderTask.nonEmpty) {
      taskGroup.filter(_.dataType == 1).foreach { task =>
        val ossPath = ossBucket + File.separator + task.dataPath
        val readconfig: JdCloudDataConfig = new JdCloudDataConfig(task.endPoint, task.accessKey, task.secretKey)
        ...
        readconfig.setObjectKey(ossPath)
        val data = JdCloudOssUtil.readDatasetFromOss(spark, readconfig)
          .withColumn("task_id", lit(task.taskId))
          .as[(String, Long, Double, Int, String, String, Long)];
    ...
    if (userTask.nonEmpty) {
      taskGroup.filter(_.dataType == 2).foreach { task =>
        ...
        readconfig.setFileType(OssFileTypeEnum.parquet)
        readconfig.setObjectKey(ossPath)
        val data = JdCloudOssUtil.readDatasetFromOss(spark, readconfig).withColumn("task_id", lit(task.taskId))
          .as[(String, Int, Double, Int, Array[String], String, String, Long)]

//        val data = JdCloudOssUtil.readDatasetFromOss(spark, task.endPoint, task.accessKey, task.secretKey, ossPath, OssFileTypeEnum.parquet, ConfigProperties.getClientType)
//          .withColumn("task_id", lit(task.taskId))
//          .as[(String, Int, Double, Int, Array[String], String, String, Long)]
    ...
    indicatorDFPre.repartition(taskGroup.length * 8).write.mode(SaveMode.Overwrite).parquet(s"$tempDir/$indicatorFile")
  ...
  def compute(spark: SparkSession,
              ...
              taskGroup: Array[RfmAnalysisTask]): Unit = {
