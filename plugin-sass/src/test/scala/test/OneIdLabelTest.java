...
import com.jd.easy.audience.task.plugin.step.dataset.labeldataset.LabelDatasetStep;
 ...
 */
public class OneIdLabelTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(OneIdLabelTest.class);
    ...
    public static void main(String[] args) {
        SparkSession sparkSession = new SparkSession.Builder()
                .master("local")
                .appName("OneIdLabelTest")
                .getOrCreate();
        SparkContext sc = sparkSession.sparkContext();
        sc.setLogLevel("ERROR");
        mockData(sparkSession);
        /**
         * userjoin.json 普通字段关联，用户标示字段为其中一张表的关联字段
         * userjoinother.json 普通字段关联，用户标示字段为其中一张表的非关联字段
         * userjoinoneid.json 普通字段关联，用户标示字段为oneid
         * oneidoinother.json 一张表oneid进行关联，但是使用其他字段为用户标示字段 -抛异常
         * oneidoinoneid.json 两张表oneid进行关联，使用oneid作为结果
         * oneidjoinotheruser.json 两张表oneid进行关联，使用其他字段作为结果
         * singleoneid.json 单个表oneid关联，其他字段为结果
         * singleuser.json 单个表oneid关联，oneid结果
         * singleuserold.json 其他字段关联，oneid为结果
         * singleuserold22.json 其他字段关联，其他字段结果
         */
        SparkStepSegment sparkSegment = getsparkstepInfo("../labeldataset/oneidjoinother.json");
        try {
            //4.任务执行
            ...
            Class c = Class.forName(stepBean.getStepClassName());
            //baoming 是该类所在的包   leiming 是一个该类的名称的String
            LOGGER.info("bean class:" + stepBean.getStepClassName());
            LabelDatasetStep stepObj = null;
            if (c.newInstance() instanceof LabelDatasetStep) {
                stepObj = (LabelDatasetStep) c.newInstance();
            } else {
                LOGGER.info("step class exception:" + stepBean.getStepClassName());
                return;
            }
            stepObj.setStepBean(stepBean);
            stepObj.validate();
            LabelDatasetBean beanObj = null;
            if (stepBean instanceof LabelDatasetBean) {
                beanObj = (LabelDatasetBean) stepBean;
            } else {
                LOGGER.info("bean class exception:" + stepBean.toString());
                return;
            }
            stepObj.dealDataSet(sparkSession, beanObj, true);
        } catch (Exception e) {
            //5.异常统一处理
            dealError(e);
        }
    ...
    private static SparkStepSegment getsparkstepInfo(String argPath) {
        ...
        try {
            //2.1
            System.out.println(OneIdLabelTest.class.getResource("").getPath());
            File file = new File(OneIdLabelTest.class.getResource("").getPath() + argPath);
            if (file.exists()) {
                try {
                    // 将文件转换成String
    ...
    private static void mockData(SparkSession sparkSession) {
        sparkSession.createDataFrame(initDatauser8(), initSchemauser8()).createOrReplaceTempView("one_id_label_8_9");
        sparkSession.createDataFrame(initDatauser9(), initSchemauser9()).createOrReplaceTempView("one_id_label_9_10");
        sparkSession.createDataFrame(initDatauseroinid(), initSchemauseroneid()).createOrReplaceTempView("cdp_one_id_test");
        sparkSession.sql("select * from one_id_label_8_9").show();
        sparkSession.sql("select * from one_id_label_9_10").show();
    ...
    private static StructType initSchemauser8() { //        StructField id = DataTypes.createStructField("id", DataTypes.IntegerType, true);
        ...
        StructField userId = DataTypes.createStructField("sex", DataTypes.StringType, true);
    ...
    private static List<Row> initDatauser8() {
        ...
        data.add(RowFactory.create("user_8_1", "user_9_1", "table_8_male"));
        data.add(RowFactory.create("user_8_2", "user_9_2", "table_8_male"));
        data.add(RowFactory.create("user_8_3", "user_9_3", "table_8_male"));
        data.add(RowFactory.create("user_8_4", "user_9_4", "table_8_male"));
        data.add(RowFactory.create("user_8_5", "", "table_8_female"));
        return data;
    }
    ...
    private static StructType initSchemauser9() { //        StructField id = DataTypes.createStructField("id", DataTypes.IntegerType, true);
        ...
        StructField userId = DataTypes.createStructField("age", DataTypes.StringType, true);
        return DataTypes.createStructType(Lists.newArrayList(venderId, venderId2, userId));
    }

    private static List<Row> initDatauser9() {
        List<Row> data = new ArrayList<Row>();
        data.add(RowFactory.create("user_10_1", "user_9_1", "table_9_age13"));
        data.add(RowFactory.create("user_10_2", "user_9_2", "table_9_age13"));
        data.add(RowFactory.create("user_10_3", "user_9_3", "table_9_age13"));
        data.add(RowFactory.create("user_10_4", "user_9_4", "table_9_age13"));
        data.add(RowFactory.create("", "user_9_5", "table_9_age12"));
        data.add(RowFactory.create("user_10_5", "", "table_9_age12"));
        return data;
    }
